...
Bảng phân chia công việc (Bảng 2)7

Bảng báo cáo phiên họp nhóm (Bảng 3)11

GIỚI THIỆU ĐỀ TÀI VÀ MÔ TẢ CHỨC NĂNG13

Giới thiệu đề tài13

Hiện trạng và vấn đề13

Mục tiêu của đề tài13

Công nghệ sử dụng14

Bảng danh mục chức năng và Endpoint API hệ thống (Bảng 4)14

DATABASE VÀ MÔ HÌNH ERD25

Mô hình ERD (Entity Relationship Diagram)25

Từ điển dữ liệu (Data Dictionary - 10 Thực thể cốt lõi)26

Bảng Users & Roles (Tài khoản và Vai trò)26

Bảng Properties / Buildings (Cơ sở lưu trú: Nhà trọ, Chung cư, Tòa nhà, Khách sạn)

Bảng Rooms & Condos (Phòng trọ, Căn hộ chung cư, Phòng khách sạn & Minibar)

Bảng Residents & Guests (Cư dân thuê trọ & Khách lưu trú khách sạn)

Bảng Contracts & Bookings (Hợp đồng thuê dài hạn & Đặt phòng khách sạn)

Bảng Utility & Services (Chốt Điện - Nước & Dịch vụ Khách sạn)

Bảng Bills & Transactions (Hóa đơn thu tiền, Bảng kê Folio & Sổ quỹ)

Bảng Tickets & RoomReports (Sự cố kỹ thuật, Dịch vụ phòng & Khiếu nại)

Bảng LandlordProfiles & VerificationRequests (Hồ sơ & Yêu cầu duyệt chủ trọ)36

Bảng AdminActivityLogs & AuditLogs (Nhật ký truy vết & Kiểm toán bất biến)37

THIẾT KẾ GIAO DIỆN DEMO VÀ KỊCH BẢN XỬ LÝ LỖI (UI/UX)39

Trang Đăng nhập & Đăng ký (WebAuthn Passkey + Mật khẩu)39

Trang Dashboard Chủ trọ / Admin (Thống kê & AI Insight)40

Trang Quản lý Danh sách phòng & Sơ đồ Ma trận phòng lưu trú (Room Matrix)

Trang Quản lý Hợp đồng thuê & Phiếu đặt phòng (Bookings) trực tuyến

Trang Ghi chỉ số Điện - Nước định kỳ & AI OCR Camera42

Trang Quản lý Thanh toán, Xuất hóa đơn VietQR & Bảng kê Folio Khách sạn (PDF)

Trang Cổng thông tin Cư dân & Khách lưu trú (Guest Portal) & Mẫu tạm trú CT01

Trang Trợ lý ảo AI & Chatbot tư vấn thuê phòng Renty44

Trang Kiểm duyệt Hồ sơ Định danh Chủ trọ (Admin Verification)44

Trang Quản lý Tài sản - Trang thiết bị & Minibar (Asset & Minibar Management)
...
Mô hình Thuê dài hạn theo tháng (Nhà trọ truyền thống & Chung cư mini):
...
Mô hình Thuê ngắn hạn theo ngày / giờ (Khách sạn & Homestay nghỉ dưỡng):
...
Khởi tạo kiến trúc dự án Laravel 11, thiết lập Docker, Git và thiết kế 35 bản ghi Database Migrations
...
Xây dựng Middleware phân quyền truy cập đa tầng RBAC & Multi-tenancy phân lập dữ liệu
...
Lập trình module Đăng ký & Đăng nhập truyền thống kèm cơ chế Rate Limiting chống brute-force
...
Tích hợp chuẩn xác thực không mật khẩu WebAuthn / FIDO2 Passkey sinh trắc học
...
Hiện thực giải pháp mã hóa dữ liệu nhạy cảm PII bằng AES-256-GCM kết hợp HMAC Blind Index
...
Xây dựng quy trình Xác thực định danh chủ trọ lũy tiến (KYC CCCD & Premium Tích xanh PCCC/ANTT)
...
Xây dựng Bảng điều khiển kiểm duyệt hồ sơ chủ trọ (Superadmin)
...
Xây dựng hệ thống Nhật ký kiểm toán bất biến (Immutable Audit Logs)
...
Tích hợp Google Gemini AI tự động phân tích và sinh điều khoản hợp đồng thuê phòng chuẩn pháp lý
...
Xây dựng quy trình Onboarding đăng ký nhanh cho chủ trọ mới
...
Lập trình module CRUD Quản lý Cơ sở lưu trú (Properties/Hotels)
...
Lập trình module CRUD Quản lý Phòng lưu trú (Rooms)
...
Thiết kế & phát triển Sơ đồ ma trận phòng trực quan (Visual Room Matrix) theo tầng
...
Quét nợ tự động và gửi tin nhắn nhắc tiền phòng kèm link VietQR qua Zalo/SMS
...
Lập trình module Quản lý Trang thiết bị - Tài sản phòng trọ (Equipment)
...
Xây dựng module Sổ quỹ thu - chi và ghi nhận dòng tiền phát sinh ngoài tiền phòng
...
Tích hợp AI tự động viết bài đăng mô tả phòng trọ chuẩn SEO thu hút khách thuê
...
Tích hợp Trợ lý ảo AI Renty Chatbot theo mô hình RAG với Google Gemini API
...
Xây dựng Cổng thông tin Cư dân & Khách lưu trú (Guest Portal)
...
Lập trình module Tiếp nhận & Xử lý sự cố kỹ thuật và Yêu cầu dịch vụ buồng phòng (Smart Tickets)
...
Lập trình module Quản lý thông tin Cư dân & Thân nhân lưu trú theo phòng
...
Lập trình chức năng Tự động trích xuất và kết xuất tờ khai đăng ký tạm trú Mẫu CT01 (Bộ Công an)
...
Xây dựng tiện ích Đăng ký nhận chuông báo khi phòng đang thuê chuyển sang trạng thái trống (Empty Room Alert)
...
Laravel Documentation (v11.x): The PHP Framework for Web Artisans. Truy cập tại: https://laravel.com/docs/11.x

Google Cloud AI Documentation: Gemini Models and OpenAI-compatible REST API Reference. Truy cập tại: https://ai.google.dev/docs

Chính phủ nước CHXHCN Việt Nam: Nghị định số 13/2023/NĐ-CP ngày 17/04/2023 về Bảo vệ dữ liệu cá nhân. Cổng thông tin điện tử Chính phủ.

Công ty Cổ phần Thanh toán Quốc gia Việt Nam (NAPAS): Tiêu chuẩn kỹ thuật định dạng thanh toán VietQR cho chuyển khoản liên ngân hàng. Truy cập tại: https://vietqr.net/

World Wide Web Consortium (W3C): Web Authentication: An API for accessing Public Key Credentials Level 2 (WebAuthn / FIDO2). Truy cập tại: https://www.w3.org/TR/webauthn-2/

Tailwind CSS Documentation: A utility-first CSS framework for rapid UI development. Truy cập tại: https://tailwindcss.com/docs

Chart.js Documentation: Simple yet flexible JavaScript charting for designers & developers. Truy cập tại: https://www.chartjs.org/docs/

Barryvdh Laravel-DomPDF: A DOMPDF Wrapper for Laravel. Truy cập tại: https://github.com/barryvdh/laravel-dompdf
