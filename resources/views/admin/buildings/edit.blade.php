...
<head>
    ...
    <meta name="description" content="Chỉnh sửa Cơ sở Lưu trú - SmartRoom.">
    <title>Chỉnh Sửa Cơ Sở Lưu Trú - SmartRoom</title>
    ...
    <div id="admin-shell" class="ml-64 min-w-0 flex flex-col h-screen overflow-y-auto relative z-10 transition-[margin-left] duration-200">
        ...
        <header class="h-16 border-b border-slate-900 bg-[#080b11]/80 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-20">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-bold text-slate-100">Chỉnh Sửa Cơ Sở Lưu Trú</h2>
                    <p class="text-xs text-slate-400">Cập nhật thông tin chi tiết: {{ $building->name }}</p>
                </div>
            </div>
        ...
        <main class="p-8 flex-grow overflow-y-auto max-w-5xl mx-auto w-full">
            ...
            <form action="{{ route('admin.buildings.update', $building->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div class="glass-card rounded-2xl p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $building->name) }}" required class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="address" value="{{ old('address', $building->address) }}" required class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $building->phone) }}" class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="total_floors" value="{{ old('total_floors', $building->total_floors ?? 1) }}" min="1" max="100" required class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">
                                <option value="active" {{ old('status', $building->status) === 'active' ? 'selected' : '' }}>Đang hoạt động</option>
                                <option value="maintenance" {{ old('status', $building->status) === 'maintenance' ? 'selected' : '' }}>Đang bảo trì / nâng cấp</option>
                                <option value="inactive" {{ old('status', $building->status) === 'inactive' ? 'selected' : '' }}>Tạm ngưng đón khách</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card rounded-2xl p-6">
                    ...
                    @php
                        ...
                        $currentAmenities = old('amenities', $building->amenities ?? []);
                        if (!is_array($currentAmenities)) $currentAmenities = [];
                    @endphp
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach($presetAmenities as $amenity)
                            <label class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-900/40 border border-slate-800 hover:border-indigo-500/40 cursor-pointer transition">
                                <input type="checkbox" name="amenities[]" value="{{ $amenity }}" {{ in_array($amenity, $currentAmenities) ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 bg-slate-800 border-slate-700 focus:ring-0">
                ...
                <div class="glass-card rounded-2xl p-6">
                    ...
                    @if($building->image)
                        <div class="mb-4">
                            <label class="block text-xs font-bold text-slate-400 mb-1.5">Ảnh hiện tại:</label>
                            <div class="w-48 h-28 rounded-xl overflow-hidden border border-slate-800 bg-slate-900">
                                <img src="{{ str_starts_with($building->image, 'http') ? $building->image : asset('storage/' . $building->image) }}" alt="Preview" class="w-full h-full object-cover">
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <p class="text-[11px] text-slate-500 mt-1">Để trống nếu muốn giữ ảnh cũ</p>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-300 mb-1.5">
                                    Hoặc cập nhật URL ảnh trực tiếp
                                </label>
                                <input type="url" name="image_url" value="{{ old('image_url', str_starts_with($building->image ?? '', 'http') ? $building->image : '') }}" placeholder="https://example.com/banner.jpg" class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" class="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500 transition">{{ old('description', $building->description) }}</textarea>
                        </div>
                    </div>
                </div>
